...
/*
 Compute a blocked TRSM on the fundamental unit.  Each diagonal processor owns one block of L and one of X.  Each off-diagonal processor owns one block of L and two of X.  Eg. on 10 processors
...
void trsm( double *X, double *L, int bs, int I, int J, int P, MPI_Comm *comms ) {
  ...
  if( I == J ) {
    ...
    for( int j = 0; j < J; j++ ) {
      ...
      recvForward( bufX1, bs2, j, comms[J] );
      localDgemm( X, bufX1, bufL1, bs );      
    }
	...
	recvForward( bufX1, bs2, j, comms[J] );
	// receive X_Ij
	recvForward( bufX2, bs2, j, comms[I] );

	localDgemm( XU, bufX1, bufL1, bs );
	localDgemm( XL, bufX2, bufL2, bs );
      } else if( j == J ) {
	// work on our block in the lower triangle
	// receive the diagonal of L
	mybcast( bufL1, bs2, J, comms[J] );
	localTrsm( bufL1, XL, bs );
	mybcast( L, bs2, J, comms[I] );
	sendForward( XL, bs2, J, P, comms[I] );
	// send the off-diagonal of L

	// one block to update
	recvForward( bufX1, bs2, j, comms[J] );
	localDgemm( XU, bufX1, L, bs );
      } else if( j < I ) {
	// one block to update
	mybcast( bufL1, bs2, j, comms[I] );
	recvForward( bufX1, bs2, j, comms[J] );
	...
	localTrsm( bufL1, XU, bs );
	sendForward( XU, bs2, I, P, comms[J] );
      }
    }
  }
  free(bufL1);
