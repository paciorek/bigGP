...
void gemm( double *C, double *A, double *B, int bs, int I, int J, int P, MPI_Comm *comms ) {
  ...
  double *bufA1 = (double*) malloc( bs2*sizeof(double) );
  double *bufB1 = (double*) malloc( bs2*sizeof(double) );
  double *bufA2 = (double*) malloc( bs2*sizeof(double) );
  double *bufB2 = (double*) malloc( bs2*sizeof(double) );
  ...
  if( I == J ) {
	...
	bB = bufB1;
      ...
      mybcast( bA, bs2, k, comms[I] );
      mybcast( bB, bs2, k, comms[I] );
      localDgemm( C, bA, bB, bs );
    }
  } else {
    for( int k = 0; k < P; k++ ) {
      double *AL = A;
	...
	bA1 = bufA1;
      mybcast( bA1, bs2, k, comms[I] );
      
      if( I == k )
	bA2 = AU;
      else
	bA2 = bufA2;
      mybcast( bA2, bs2, k, comms[J] );

      if( J == k )
	bB1 = BL;
      else
	bB1 = bufB1;
      mybcast( bB1, bs2, k, comms[I] );
      
      if( I == k )
	bB2 = BU;
      else
	bB2 = bufB2;
      mybcast( bB2, bs2, k, comms[J] );
      
      localDgemm( CL, bA1, bB2, bs );
      localDgemm( CU, bA2, bB1, bs );
    }
  }
  free(bufA1);
  free(bufA2);  
